...
@interface WMPageController () <WMMenuViewDelegate,UIScrollViewDelegate,SlideNavigationControllerDelegate> {
    CGFloat viewHeight;
    CGFloat viewWidth;
    CGFloat targetX;
    BOOL    animate;
...
- (void)layoutChildViewControllers {
    ...
    } else if (currentPage + 1 == self.titles.count){
        start = currentPage - 1;
        end = currentPage;
    } else {
        start = currentPage - 1;
...
//    UIViewController *viewController = [[vcClass alloc] init];
    ...
    ColumnController *viewController = [mainSB instantiateViewControllerWithIdentifier:@"ColumnController"];
//    viewController.cateLink
    viewController.cateLink = self.columnLinks[index];
//    NSLog(@"come on baby%@",self.columnLinks);
